...
const Footer: React.FC<FooterProps> = ({ language }) => {
  ...
  return (
    <footer className="bg-[#16425B] text-white py-16 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid md:grid-cols-3 gap-12">
          {/* Left Section */}
          <div>
            <h3 className="text-2xl font-bold mb-4">Dr. Kaleab Nigusse</h3>
            <p className="text-white/80 leading-relaxed">{content[language].tagline}</p>
            
            {/* Bouncing Stethoscope */}
            <div className="mt-6 flex items-center gap-3">
              <div className="w-8 h-8 text-[#81C3D7] animate-bounce">
              ...
              <div className="text-sm text-white/60">Always Caring, Always Creating</div>
            </div>
          </div>

          {/* Center Section */}
          <div>
            <h4 className="text-lg font-semibold mb-6">{content[language].quickLinks}</h4>
            <div className="space-y-3">
              {content[language].links.map((link, index) => (
                <a
                  key={index}
                  href="#"
                  className="block text-white/80 hover:text-[#81C3D7] transition-colors duration-200 hover:translate-x-2 transform transition-transform"
                >
                  {link}
                </a>
              ))}
          ...
          {/* Right Section */}
          <div>
            <h4 className="text-lg font-semibold mb-6">{content[language].social}</h4>
            <div className="flex gap-4">
              <a href="#" className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center hover:bg-[#81C3D7] hover:scale-110 transition-all duration-300">
